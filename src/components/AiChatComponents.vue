...
const handleStream = async (reader, id) => {
  ...
  const readStream = async () => {
    ...
    if (done) {
      ...
      if (index !== -1) {
        messages.value[index].content = messageBuffer;
      }
      return;
    }
...
const sendMessage = async () => {
  ...
  try {
    const id = Date.now();
    const userId = 'nuecout';
    // 修改请求URL部分
    const response = await fetch(
      `http://localhost:8080/api/saleAgent?userId=${encodeURIComponent(userId)}&message=${encodeURIComponent(message)}`,
      {
        method: 'GET',
        mode: 'cors',
        credentials: 'include',
